...
export const change: ContentModule = {
  ...
  rules: [
    ...
    },
    {
      ...
      params: { pay: [1, 4], cost: [1, 9] },
      answer: (p) => ((p.pay as number) * 5 - (p.cost as number)) * 1000,
      text: (p) => `Ana buys a toy for ${rp((p.cost as number) * 1000)}. She pays ${rp((p.pay as number) * 5000)}. Change?`,
      exclude: (p) => (p.cost as number) >= (p.pay as number) * 5,
      distractors: 'near',
    },
    {
      ...
      params: { pay: [1, 4], cost: [1, 9] },
      answer: (p) => ((p.pay as number) * 5 - (p.cost as number)) * 1000,
      text: (p) => `Budi pays ${rp((p.pay as number) * 5000)} for a book that costs ${rp((p.cost as number) * 1000)}. Change?`,
      exclude: (p) => (p.cost as number) >= (p.pay as number) * 5,
      distractors: 'near',
    },
  ],
