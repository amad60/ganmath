...
export const shopping: ContentModule = {
  ...
  rules: [
    ...
    },
    {
      ...
      params: { n: [2, 9], c: [1, 9] },
      answer: (p) => (p.n as number) * (p.c as number) * 1000,
      text: (p) => `Ana buys ${p.n} toys at ${rp((p.c as number) * 1000)} each. What is the total?`,
      distractors: 'near',
    },
    {
      type: 'choose-number',
      ...
      params: { n: [2, 9], c: [1, 9] },
      answer: (p) => (p.n as number) * (p.c as number) * 1000,
      text: (p) => `Budi buys ${p.n} books. Each book costs ${rp((p.c as number) * 1000)}. What is the total?`,
      distractors: 'near',
    },
  ],
};
